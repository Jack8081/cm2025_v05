...
static const struct media_memory_block media_memory_config[] = {
...

    {
		...
		.mem_cell = {
			...
			{.mem_type = AI_PARAM, .mem_base = (uint32_t)&playback_input_buffer[0x1700], .mem_size = 0x800,}, //tmp used at startup
			{.mem_type = DAE_PRE_PARAM,  .mem_base = (uint32_t)&playback_input_buffer[0x2100], .mem_size = sizeof(dae_para),}, //0x380
			{.mem_type = DMVP_PARAM, .mem_base = (uint32_t)&playback_input_buffer[0x2480], .mem_size = 0xF74,},

			//{.mem_type = NEW_DEBUG_DUMP_BUF, .mem_base = (uint32_t)&aset_dump_buffer[0], .mem_size = sizeof(aset_dump_buffer),},
			{.mem_type = NEW_DEBUG_DUMP_BUF, .mem_base = (uint32_t)&playback_input_buffer[0x3400], .mem_size = 0x600,},
		},
	},
};
