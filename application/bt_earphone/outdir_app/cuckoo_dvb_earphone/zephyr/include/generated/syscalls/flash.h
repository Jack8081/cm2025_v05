...
extern int z_impl_flash_flush(const struct device * dev, bool efficient);

__pinned_func
static inline int flash_flush(const struct device * dev, bool efficient)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int) arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&efficient, K_SYSCALL_FLASH_FLUSH);
	}
#endif
	compiler_barrier();
	return z_impl_flash_flush(dev, efficient);
}
